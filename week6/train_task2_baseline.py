...
def train(
    ...
    wandb.log(data={
        'Train Loss': loss_iter,
        'Train Loss mean': loss_train_mean(loss_iter),
        'Train Acc': (float(hits_iter) / len(labels)),
        'Train Acc mean': (float(hits) / count)
    }, step=epoch)

    return loss_train_mean(loss_iter)
...
def evaluate(
    ...
    if log:
        wandb.log(data={
            'Val Loss': loss_iter,
            'Val Loss mean': loss_valid_mean(loss_iter),
            'Val Acc': (float(hits_iter) / len(labels)),
            'Val Acc mean': (float(hits) / count)
        }, step=epoch)
...
if __name__ == "__main__":
    ...
    print("Clip length 8 - shuffle")
    print(" length", args.clip_length)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.validate_every == 0:
            ...
            evaluate(model, loaders['validation'], loss_fn, args.device, epoch, description=description, log=True)
        # Training
        description = f"Training [Epoch: {epoch+1}/{args.epochs}]"
        train_loss = train(model, loaders['training'], optimizer, loss_fn, args.device, epoch, description=description)
        ...
        if train_loss < best_val_loss - min_delta:
            ...
            model_path = './weights_clip_length_8_shuffle.pth'
