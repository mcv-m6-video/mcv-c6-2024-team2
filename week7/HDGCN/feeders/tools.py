...
def crop_resize(data, clip_length, temporal_stride):
    seg_len = data.shape[1]
    C, T, V, M = data.shape

    if seg_len >= clip_length * temporal_stride:
        # Randomly select a clip from the video with the desired length (start and end frames are inclusive)
        clip_begin = random.randint(0, max(seg_len - clip_length * temporal_stride, 0))
        clip_end = clip_begin + clip_length * temporal_stride
        data = data[:, clip_begin:clip_end:temporal_stride]
        data = torch.tensor(data,dtype=torch.float)
    else:
        print(data.shape)
        _, original_dim, _, _ = data.shape
        pad_needed = max(0, 5 - original_dim)
        padding = (0, 0, 0, 0, 0, pad_needed, 0, 0)
        data = torch.tensor(data,dtype=torch.float)

        data = F.pad(data, padding)
        # curr_len = data.shape[1]
        # zero_array = np.zeros((17, 1))
        # for i in range(clip_length - curr_len + 1):
        #     for j in range(data.shape[0]):
        #         np.concatenate(data[j], zero_array)
        #         print(data.shape)
    ...
    # TF does this do??
    # data = data.permute(0, 2, 3, 1).contiguous().view(C * V * M, clip_length)
    # data = data[None, None, :, :]
    # data = F.interpolate(data, size=(C * V * M, clip_length), mode='bilinear',align_corners=False).squeeze() # could perform both up sample and down sample
    # data = data.contiguous().view(C, V, M, clip_length).permute(0, 3, 1, 2).contiguous().numpy()
