...
class HMDB51Dataset(Dataset):
    ...
    def _segment_video(self, video_len, frame_paths):
        frames_per_segment = video_len // self.segment_number


        segments_paths = []
        start_idx = 0
        for i in range(self.segment_number):  # Loop for 3 segments
            end_idx = min(start_idx + frames_per_segment, video_len)
            segment_frames = frame_paths[start_idx:end_idx]

            segments_paths.append(segment_frames)
            start_idx = end_idx

        return segments_paths

    def _get_clip(self, segment):
        seg_len = len(segment)

        if seg_len <= self.clip_length * self.temporal_stride:
            # Not enough frames to create the clip
            clip_begin, clip_end = 0, seg_len
        else:
            # Randomly select a clip from the video with the desired length (start and end frames are inclusive)
            clip_begin = random.randint(0, max(seg_len - self.clip_length * self.temporal_stride, 0))
        ...
        clip = None
        for i, path in enumerate(segment[clip_begin:clip_end:self.temporal_stride]):
            frame = read_image(path)  # (C, H, W)
            if clip is None:
                clip = torch.zeros((self.clip_length, 3, frame.shape[1], frame.shape[2]), dtype=torch.uint8)
            clip[i] = frame
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        if self.regime.value == 1 or self.regime.value == 3:
            # Segment video in equally sized segments
            segments = self._segment_video(video_len=video_len, frame_paths=frame_paths)

            # Randomly sample one clip/view in each
            clips = [self._get_clip(seg) for seg in segments]
        else:
            clips = self._get_clip(frame_paths)
    ...
    def collate_fn(self, batch: list) -> dict:
        ...
        if self.regime.value == 1 or self.regime.value == 3:
            ...
            # Each video_sample contains a number of clips equal to self.segment_number
            transformed_clips = []
            for video_sample in unbatched_clips:
                transformed_clips.append([self.transform(clip).permute(1, 0, 2, 3) for clip in video_sample])
                batched_clips = transformed_clips
        else:
            # Apply transformation and permute dimensions: (T, C, H, W) -> (C, T, H, W)
